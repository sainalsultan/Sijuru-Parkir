...
package com.kioser.profile.camera.v1
...
object CameraV1Util {
    ...
    fun getBestPreviewSizeMine(
        ...
        if (sizes != null) {
            for (size in sizes.sortedByDescending { it.height }.filter { it.width != it.height }) {
                Log.d(TAG,"size ${size.width} x ${size.height}")
                if(size.height in 720..1200) {
                    if (mSize == null) {
                        mSize = size
                    }else if (size.width > mSize.width){
                        mSize = size
                    }
                }
            }
